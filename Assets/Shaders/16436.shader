Shader "Custom/16436"
{
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			{
				fixed4 color = _Color + tex2D(_Texture1, IN.uv_Texture1);
				color *= tex2D(_Texture2, IN.uv2_Texture2) * 2;
				OUT.Albedo = color.rgb;
				fixed4 alpha = _Color + tex2D(_Texture1, IN.uv_Texture1);
				alpha *= tex2D(_Texture2, IN.uv2_Texture2) * 2;
				OUT.Alpha = alpha.a;
				OUT.Metallic = 0;
				OUT.Smoothness = 0;
			}
